...
export function sumPoints(questions: Question[]): number {
    ...
    let sum = 0;
    scores.forEach((points: number): number => (sum += points));
    return sum;
}
...
export function sumPublishedPoints(questions: Question[]): number {
    const scores = questions.map((question: Question): number =>
        question.published === true ? question.points : 0
    );
    let sum = 0;
    scores.forEach((points: number): number => (sum += points));
    return sum;
...
export function toCSV(questions: Question[]): string {
    ...
    const index = csv.length - 1;
    csv[index] = csv[index].slice(0, -1);
...
export function sameType(questions: Question[]): boolean {
    ...
    questions.map((question: Question): boolean =>
        question.type === first.type ? (truth = true) : (truth = false)
    );
    return truth;
}
    ...
    return questions.map((question: Question): Question => {
        if (question.id === targetId) {
            if (targetOptionIndex === -1) {
                ...
                const replaced = [...question.options];
...
export function duplicateQuestionInArray(
    ...
    dupes.map((question: Question): void => {
        if (question.id === targetId) {
            dupes.splice(
                dupes.indexOf(question) + 1,
                0,
